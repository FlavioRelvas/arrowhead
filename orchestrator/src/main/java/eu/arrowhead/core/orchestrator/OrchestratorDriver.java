...
import eu.arrowhead.common.messages.QoSReservationForm;
...
import eu.arrowhead.core.orchestrator.support.QoSRequestForm;
...
/**
 * Contains miscellaneous helper functions for the Orchestration process. The
 * main functions of the Orchestration process used by the REST resource are in
 * {@link eu.arrowhead.core.orchestrator.OrchestratorService}.
 *
...
final class OrchestratorDriver {
    ...
    /**
     * Queries the Service Registry Core System for a specific
     * <tt>ArrowheadService</tt>.
     *
     * @param service The <tt>ArrowheadService</tt> object for which the list of
     * potential <tt>ArrowheadSystem</tt> providers are needed
     * @param metadataSearch If true, the stored <tt>ArrowheadService</tt>s have
     * to have the same metadata to be returned in the response.
     * @param pingProviders If true, the Service Registry is asked to ping the
     * service provider <tt>ArrowheadSystem</tt> (where the service is offered)
     * to check if a connection can be established or not. Normally providers
     * have to remove their offered services from the Service Registry before
     * going offline, but this feature can be used to ensure offline providers
     * are filtered out.
     *
     * @return list of potential service providers with their offered services
     * (interfaces, metadata, service URI)
     *
     * @throws DataNotFoundException if the Service Registry response list is
     * empty
     */
    ...
    /**
     * Queries the Authorization Core System to see which provider
     * <tt>ArrowheadSystem</tt>s are authorized to offer their services to the
     * consumer.
     *
     * @param consumer The <tt>ArrowheadSystem</tt> object representing the
     * consumer system
     * @param service The <tt>ArrowheadService</tt> object representing the
     * service to be consumed
     * @param providerSet The set of <tt>ArrowheadSystem</tt> objects
     * representing the potential provider systems
     *
     * @return list of the authorized provider <tt>ArrowheadSystem</tt>s
     *
     * @throws DataNotFoundException if none of the provider
     * <tt>ArrowheadSystem</tt>s are authorized for this servicing
     */
    ...
    /**
     * Filters out all the entries of the given <tt>ServiceRegistryEntry</tt>
     * list, which does not contain a preferred local <tt>ArrowheadSystem</tt>.
     * This method is called when the <i>onlyPreferred</i> orchestration flag is
     * set to true.
     *
     * @param srList The list of <tt>ServiceRegistryEntry</tt>s still being
     * considered (after SR query and possibly Auth query)
     * @param preferredLocalProviders A set of local <tt>ArrowheadSystem</tt>s
     * preferred by the requester <tt>ArrowheadSystem</tt>. This is a subset of
     * the <i>preferredProviders</i> list from the {@link ServiceRequestForm},
     * which can also contain not local <tt>ArrowheadSystem</tt>s.
     *
     * @return a list of <tt>ServiceRegistryEntry</tt>s which have preferred
     * provider <tt>ArrowheadSystem</tt>s
     *
     * @throws DataNotFoundException if none of the
     * <tt>ServiceRegistryEntry</tt>s from the given list contain a preferred
     * <tt>ArrowheadSystem</tt>
     */
    ...
    static List<ServiceRegistryEntry> doQoSVerification(List<ServiceRegistryEntry> srList, ArrowheadSystem clientSystem, Map<String, String> qosParameters) {
        //placeholder for actual implementation
        String uri = UriBuilder.fromPath(OrchestratorMain.QOS_MANAGER_URI).path("verify").toString();
        //Construct payload to send to QoSManager Service

        List<ArrowheadSystem> providers = new ArrayList<>();
        for (ServiceRegistryEntry entry : srList) {
            providers.add(entry.getProvider());
        }
        QoSRequestForm qrf = new QoSRequestForm.Builder(clientSystem, providers, qosParameters).build();
        System.out.println("Request to " + uri + " with payload: " + Utility.toPrettyJson(null, qrf));
    ...
    /**
     * As the last step of the local orchestration process (if requested with
     * the <i>matchmaking</i> orchestration flag) we pick out 1 provider from
     * the remaining list. Providers preferred by the consumer have higher
     * priority. Custom matchmaking algorithm can be implemented here, as of now
     * it just returns the first (preferred) provider from the list.
     * <p>
     * If the <i>onlyPreferred</i> orchestration flag is set to true, then it is
     * guaranteed there will be at least 1 preferred provider to choose from,
     * since this method is called after {@link #removeNonPreferred(List, Set)},
     * where a {@link eu.arrowhead.common.exception.DataNotFoundException} is
     * thrown if no preferred provider was found.
     *
     * @param srList The list of <tt>ServiceRegistryEntry</tt>s still being
     * considered
     * @param preferredLocalProviders The set of <tt>ArrowheadSystem</tt>s in
     * this Local Cloud preferred by the requester system
     *
     * @return the chosen ServiceRegistryEntry object, containing the necessary
     * <tt>ArrowheadSystem</tt> and <tt>String</tt> serviceUri information to
    ...
    /**
     * Queries the Orchestration Store database table for a consumer
     * <tt>ArrowheadSystem</tt>. The Orchestration Store holds <i>hardwired</i>
     * <tt>ArrowheadService</tt>s between consumer and provider
     * <tt>ArrowheadSystem</tt>s. The provider system can be local or part of
     * another cloud. For more information see
     * {@link eu.arrowhead.common.database.OrchestrationStore}.
     *
     * @param consumer The <tt>ArrowheadSystem</tt> object representing the
     * consumer system (mandatory)
     * @param service The <tt>ArrowheadService</tt> object representing the
     * service to be consumed (optional)
     *
     * @return a list of <tt>OrchestrationStore</tt> objects matching the query
     * criteria
     *
    ...
    /**
     * Cross-checks the query results from the <i>Orchestration Store</i> with
     * the <i>Service Registry</i> and <i>Authorization</i>. A provider
     * <tt>ArrowheadSystem</tt> has to be registered into the <i>Service
     * Registry</i> at the time of the servicing request while being authorized
     * too.
     *
     * @param srf The <tt>ServiceRequestForm</tt> from the requester
     * <tt>ArrowheadSystem</tt>
     * @param entryList Result of the <i>Orchestration Store</i> query. All the
     * entries matching the criteria provided by the
     * <tt>ServiceRequestForm</tt>
     *
     * @return the list of <tt>OrchestrationStore</tt> objects which remained
     * from the query after the cross-check
     */
    ...
    /**
     * Initiates the Global Service Discovery process by sending a request to
     * the Gatekeeper Core System.
     *
     * @param requestedService The <tt>ArrowheadService</tt> object representing
     * the service for which the Gatekeeper will try to find a provider system
     * @param preferredClouds A list of <tt>ArrowheadCloud</tt>s which are
     * preferred by the requester system for service consumption. If this list
     * is empty, the Gatekeeper will send GSD poll requests to the
     * <tt>NeighborCloud</tt>s instead.
     *
     * @return the GSD result from the Gatekeeper Core System
     *
     * @throws DataNotFoundException if none of the discovered
     * <tt>ArrowheadCloud</tt>s returned back positive result
     */
    ...
    /**
     * Inter-Cloud matchmaking is mandatory for picking out a target Cloud to do
     * ICN with. Clouds preferred by the consumer have higher priority. Custom
     * matchmaking algorithm can be implemented, as of now it just returns the
     * first Cloud from the list.
     *
     * @param result The <tt>GSDResult</tt> object contains the
     * <tt>ArrowheadCloud</tt>s which responded positively to the GSD polling.
     * @param preferredClouds The <tt>ArrowheadCloud</tt>s preferred by the
     * requester <tt>ArrowheadSystem</tt>.
     * @param onlyPreferred An orchestration flags, indicating whether or not
     * the requester <tt>ArrowheadSystem</tt> only wants to consume the
     * <tt>ArrowheadService</tt> from a preferred provider.
     *
     * @return the target <tt>ArrowheadCloud</tt> for the ICN process
     *
     * @throws DataNotFoundException if there is no preferred provider Cloud
     * available while <i>onlyPreferred</i> is set to true
     */
    ...
    /**
     * Compiles an <tt>ICNRequestForm</tt> from the given parameters and
     * initiates the Inter Cloud Negotiations process by sending a request to
     * the Gatekeeper Core System. The <tt>ICNRequestForm</tt> is a complex
     * object containing all the necessary information to create a
     * <tt>ServiceRequestForm</tt> at the remote cloud.
     *
     * @param srf The <tt>ServiceRequestForm</tt> sent in by the requester
     * <tt>ArrowheadSystem</tt>. 4 different fields of it is used in this
     * method.
     * @param targetCloud The <tt>ArrowheadCloud</tt> entity this local cloud
     * chose to do ICN with.
     *
     * @return a boxed {@link OrchestrationResponse} object from the remote
     * cloud
     *
     * @throws DataNotFoundException if the ICN failed with the remote cloud for
     * some reason
     */
     ...
     * orchestration process (if requested) we pick out 1 provider from the ICN
     * result list. Providers preferred by the consumer have higher priority.
     * Custom matchmaking algorithm can be implemented, as of now it just
     * returns the first provider from the list.
     *
     * @throws DataNotFoundException in case of Store orchestration, and the
     * provider system from the database is not a match according to the remote
     * cloud
     */
    ...
    /**
     * Requests <tt>ArrowheadToken</tt> generation from the Authorization Core
     * System for <tt>ArrowheadService</tt>s, where the metadata contains the
     * "security-token" key-pair. The consumer <tt>ArrowheadSystem</tt>s will
     * use these credentials to contact the provider <tt>ArrowheadSystem</tt>s
     * (if the providers are operating in a secure manner).
     *
     * @param srf The <tt>ServiceRequestForm</tt> sent in by the requester
     * <tt>ArrowheadSystem</tt>. 3 different fields of it is used in this
     * method.
     * @param ofList The <tt>OrchestrationForm</tt> list the Orchestrator will
     * send back.
     *
     * @return the same <tt>OrchestrationForm</tt> list supplemented with the
     * generated <tt>ArrowheadToken</tt>s for providers
     */
